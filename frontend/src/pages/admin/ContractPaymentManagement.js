...
const ContractPaymentManagement = () => {
    ...
    const formatDate = (dateString) => {
        ...
        // Если строка уже в ISO-формате (например, "2025-12-13"), используем как есть
        ...
        // Иначе предполагаем формат DD.MM.YYYY
        const parts = dateString.split(".");
        if (parts.length === 3) {
            const [day, month, year] = parts;
            // Обратите внимание: месяц в JS начинается с 0, поэтому month - 1
            const date = new Date(`${year}-${month}-${day}`);
            if (isNaN(date.getTime())) {
                return "—"; // или '' если дата некорректна
            }
    ...
    return (
        ...
        >
            <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 4 }}>
                ...
                >
                    Управление платежами по договорам
                </Typography>
            </Box>
                                ...
                                {payments.length === 0 ? (
                                    ...
                                    payments.map((payment) => (
                                        ...
                                        >
                                            ...
                                            <TableCell>
                                                {payment.rental_contract
                                                    ?.contract_number || "N/A"}
                                            </TableCell>
                                            <TableCell>
                                                {payment.rental_contract?.film
                                                    ?.title || "N/A"}
                                            </TableCell>
                                            <TableCell>
                                                {payment.rental_contract
                                            ...
                                            <TableCell>
                                                {payment.rental_contract?.cinema
                                                    ?.name || "N/A"}
                                            </TableCell>
                                            <TableCell
                                                sx={{
                                            ...
                                            <TableCell>
                                                {payment.payment_document_number ||
                                                    "—"}
                                            </TableCell>
                                            <TableCell>
                                                <Box
