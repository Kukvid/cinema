...
const PromocodesManage = () => {
        ...
        if (promocode) {
            reset({
                ...
                valid_until: promocode.valid_until
                    ...
                    : "",
                applicable_category: promocode.applicable_category || "",
    ...
    const onSubmit = async (data) => {
        try {
            ...
            // For creation, validate required date fields
            if (!editingPromocode) {
                if (!data.valid_from || !data.valid_until) {
                    setError(
                        "Даты начала и окончания действия обязательны для создания промокода"
                    );
                    return;
                }
            }
            ...
            setError(
                err.response?.data?.detail || "Не удалось сохранить промокод"
            );
            console.error(err);
        } finally {
    ...
    const handleDelete = async (id, code) => {
        if (
            !window.confirm(
                `Вы уверены, что хотите удалить промокод "${code}"?`
            )
        ) {
            return;
        }
        ...
        try {
            ...
            setError(
                err.response?.data?.detail || "Не удалось удалить промокод"
            );
            console.error(err);
        }
    };
    ...
    return (
        ...
        >
            ...
            <TableContainer
                component={Paper}
                sx={{ background: "rgba(31, 31, 31, 0.8)" }}
            >
                <Table>
                    <TableHead>
                        <TableRow>
                    ...
                    <DialogContent>
                        ...
                        >
                            ...
                            <TextField
                                ...
                                helperText={
                                    errors.description?.message || "Опционально"
                                }
                            />
                            ...
                            <TextField
                                ...
                                helperText={
                                    ...
                                    "Выберите дату окончания действия"
                                }
                            />

                            <TextField
                                margin="dense"
                                label="Применимая категория"
                                type="text"
                                fullWidth
                                variant="outlined"
                                {...register("applicable_category", {
                                    maxLength: {
                                        value: 100,
                                        message:
                                ...
                                error={!!errors.applicable_category}
                                helperText={
                                    errors.applicable_category?.message ||
                                    "Опционально (например: TICKETS, CONCESSIONS, ORDER)"
                                }
                            />
                            <FormControl
                                fullWidth
