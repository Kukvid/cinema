# Quick Start Guide - Cinema Management System

## Быстрый старт (5 минут)

### 1. Установка зависимостей
```bash
cd backend
pip install -r requirements.txt
```

### 2. Настройка .env
Создайте файл `.env` в директории `backend/`:
```env
DATABASE_URL=postgresql+asyncpg://postgres:your_password@localhost:5432/cinema_db
SECRET_KEY=your-secret-key-here
DEBUG=True
DB_ECHO=False
```

### 3. Создание базы данных PostgreSQL
```bash
# Подключитесь к PostgreSQL
psql -U postgres

# Выполните команду
CREATE DATABASE cinema_db;
\q
```

### 4. Инициализация схемы БД
```bash
python create_db.py
```
Введите `yes` для подтверждения.

### 5. Заполнение тестовыми данными
```bash
python seed.py
```

### 6. Запуск сервера
```bash
uvicorn app.main:app --reload
```

Готово! Откройте http://localhost:8000/docs для просмотра API документации.

---

## Тестовые учетные данные

### Администратор
- Email: `admin@cinema.ru`
- Password: `admin123`

### Менеджер
- Email: `manager@cinema.ru`
- Password: `manager123`

### Пользователи
- Email: `user@example.com`, `ivan@example.com`, `maria@example.com`, и др.
- Password: `user123` (для всех)

---

## Что создано?

✅ **3 кинотеатра** в разных городах (Москва, СПб, Казань)
✅ **6-9 залов** с местами (8 рядов × 12 мест = 96 мест на зал)
✅ **20 фильмов** (от новинок до классики)
✅ **50+ сеансов** на ближайшие 7 дней
✅ **8 пользователей** с бонусными счетами
✅ **7 промокодов** (WELCOME10, NEWYEAR2024, BIRTHDAY20, и др.)
✅ **57 товаров кинобара** (попкорн, напитки, снеки)
✅ **10-15 тестовых заказов** с билетами и QR кодами

---

## Активные промокоды

| Код | Скидка | Минимальная сумма |
|-----|--------|-------------------|
| WELCOME10 | 10% | 500₽ |
| NEWYEAR2024 | 500₽ | 1000₽ |
| BIRTHDAY20 | 20% | 300₽ |
| WEEKEND15 | 15% | 600₽ |
| STUDENT | 200₽ | 400₽ |

---

## Полезные команды

### Пересоздать БД с нуля
```bash
python create_db.py  # Пересоздать схему
python seed.py       # Заполнить данными
```

### Проверить подключение к БД
```bash
psql -U postgres -d cinema_db -c "\dt"
```

### Просмотреть данные
```bash
psql -U postgres cinema_db
SELECT * FROM users;
SELECT * FROM films;
\q
```

---

## Структура проекта

```
backend/
├── app/
│   ├── models/          # SQLAlchemy модели
│   ├── schemas/         # Pydantic схемы
│   ├── services/        # Бизнес-логика
│   ├── routers/         # API endpoints
│   ├── utils/           # Утилиты (security, qr_generator)
│   ├── database.py      # Настройка БД
│   ├── config.py        # Конфигурация
│   └── main.py          # FastAPI приложение
├── create_db.py         # Скрипт создания схемы БД
├── seed.py              # Скрипт заполнения тестовыми данными
├── requirements.txt     # Python зависимости
├── .env                 # Конфигурация (создать вручную)
├── README_SETUP.md      # Подробная инструкция
└── QUICKSTART.md        # Этот файл
```

---

## API Endpoints

После запуска сервера доступны:

- **Документация Swagger UI:** http://localhost:8000/docs
- **ReDoc:** http://localhost:8000/redoc
- **OpenAPI Schema:** http://localhost:8000/openapi.json

### Основные эндпоинты:

**Аутентификация:**
- `POST /api/auth/login` - Вход
- `POST /api/auth/register` - Регистрация
- `POST /api/auth/refresh` - Обновление токена

**Фильмы:**
- `GET /api/films` - Список фильмов
- `GET /api/films/{id}` - Информация о фильме
- `GET /api/films/{id}/sessions` - Сеансы фильма

**Сеансы:**
- `GET /api/sessions` - Список сеансов
- `GET /api/sessions/{id}` - Информация о сеансе
- `GET /api/sessions/{id}/seats` - Места в зале

**Заказы:**
- `POST /api/orders` - Создать заказ
- `GET /api/orders/{id}` - Информация о заказе
- `GET /api/orders/my` - Мои заказы

**Билеты:**
- `POST /api/tickets` - Купить билет
- `GET /api/tickets/{id}` - Информация о билете
- `POST /api/tickets/{id}/validate` - Проверить билет

**Промокоды:**
- `POST /api/promocodes/validate` - Проверить промокод
- `GET /api/promocodes` - Список промокодов (admin)

**Кинобар:**
- `GET /api/concessions` - Товары кинобара
- `POST /api/concessions/preorder` - Предзаказ

**Бонусы:**
- `GET /api/bonuses/account` - Мой бонусный счет
- `GET /api/bonuses/transactions` - История операций

---

## Примеры запросов

### Вход в систему
```bash
curl -X POST "http://localhost:8000/api/auth/login" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "password": "user123"
  }'
```

### Получить список фильмов
```bash
curl "http://localhost:8000/api/films"
```

### Получить сеансы на сегодня
```bash
curl "http://localhost:8000/api/sessions?date=2024-11-23"
```

### Применить промокод
```bash
curl -X POST "http://localhost:8000/api/promocodes/validate" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "code": "WELCOME10"
  }'
```

---

## Troubleshooting

### Ошибка: "Connection refused"
➡️ Проверьте, что PostgreSQL запущен:
```bash
# Windows: services.msc → PostgreSQL
# Linux: sudo systemctl status postgresql
# macOS: brew services list
```

### Ошибка: "Database does not exist"
➡️ Создайте БД:
```bash
psql -U postgres -c "CREATE DATABASE cinema_db;"
```

### Ошибка: "ModuleNotFoundError"
➡️ Установите зависимости:
```bash
pip install -r requirements.txt
```

### Seed скрипт долго выполняется
➡️ Это нормально. Генерация QR кодов и хеширование паролей занимает 10-30 секунд.

---

## Следующие шаги

1. ✅ Изучите API документацию: http://localhost:8000/docs
2. ✅ Проверьте созданные данные в БД
3. ✅ Протестируйте API эндпоинты
4. ✅ Подключите frontend приложение
5. ✅ Настройте CORS для вашего frontend

---

## Полезные ссылки

- **Подробная инструкция:** [README_SETUP.md](README_SETUP.md)
- **FastAPI документация:** https://fastapi.tiangolo.com/
- **SQLAlchemy документация:** https://docs.sqlalchemy.org/
- **PostgreSQL документация:** https://www.postgresql.org/docs/

---

**Готово к работе!** 🎬🍿
